...
"""
This provides a webpage to label neurons in a given dataset.
...
def get_rgba_volume(t: int) -> np.ndarray:
    ...
    if raw_vol.ndim == 4:
        ...
        if shape_c == 2:
            blue = np.zeros((1, *shape_zyx), dtype=np.uint8)
            rgb = np.concatenate((raw_vol, blue))

        else:
            rgb = raw_vol

    else:
        g = np.expand_dims(raw_vol, 0)
        shape_zyx = g.shape[1:]
        r = np.zeros((1, *shape_zyx), dtype=np.uint8)
